...
impl Writer for RedHatWriter {
    ...
    fn generate_config(&self, iface: &String, interface: &InterfaceType) -> FileToWrite {
        // TODO clearly this needs to be a thing we do once not twice
        let mut path_str = String::from("/etc/sysconfig/network-scripts/ifcfg-");
        path_str.push_str(iface);
        let filename = PathBuf::from(path_str);
        let content;
        match interface {
...
ONBOOT=yes
NM_CONTROLLED=no
TYPE=Ethernet", bullet="Generated by gleam", iface=iface) },
            &Static(ref network) => {
...
NETMASK={netmask}
ONBOOT=yes
NM_CONTROLLED=no", bullet="Generated by gleam", iface=iface, ip_address=network.ip_address, netmask=network.netmask, hwaddr=network.link);
            }
        };
        return FileToWrite {
            path: filename,
